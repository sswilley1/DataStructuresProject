...
public class GameChoice extends JFrame implements ActionListener  {
	...
	//this is what i seem to be having a problem with but not sure
	//might need someone to look it over
	//but i think its in the right direction. maybe?
	public static void clickButton(ActionEvent event) {
		if(event.getSource() == MegaMillionsButton) {
			
			JOptionPane.showMessageDialog(frame, "Great choice! Please enter numbers on next screen.");
		}else if(event.getSource() == Fantasy5Button) {
			
			JOptionPane.showMessageDialog(frame, "Great choice! Please enter numbers on next screen.");
		} else if(event.getSource() == PowerballButton) {
